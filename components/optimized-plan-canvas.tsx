...
interface OptimizedPlanCanvasProps {
  pdfUrl: string
  drawings: Drawing[]
...
export default function OptimizedPlanCanvas({
  pdfUrl,
  drawings,
  onDrawingsChange,
  rightSidebarOpen,
  ...
  // Load PDF and render pages as canvas elements (but with better memory management)
  useEffect(() => {
    const loadPdf = async () => {
      try {
        ...
        // Render pages at optimal scale for current zoom
        ...
        for (let i = 1; i <= pdf.numPages; i++) {
          ...
          currentOffset += viewport.height + 20 // 20px gap between pages
