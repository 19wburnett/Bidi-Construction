...
interface LightweightPlanCanvasProps {
  pdfUrl: string
  drawings: Drawing[]
...
export default function LightweightPlanCanvas({
  pdfUrl,
  drawings,
  onDrawingsChange,
  rightSidebarOpen,
        ...
        >
          {Array.from({ length: numPages }, (_, index) => (
            <div key={index} className="mb-5">
              <iframe
                src={`${pdfUrl}#page=${index + 1}&toolbar=0&navpanes=0&scrollbar=0`}
                width="800"
                height="600"
                style={{
                  border: 'none',
